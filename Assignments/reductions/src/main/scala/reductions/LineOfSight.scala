...
object LineOfSight {
   ...
   */
  def upsweepSequential(input: Array[Float], from: Int, lclUntil: Int): Float = {
    val angles = for {
      i <- from until lclUntil
    } yield {
      if (i == 0) 0
      else input(i) / i
    }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (size > threshold) {
      // return node
      val halfSize = size / 2
      val (left, right) = parallel( upsweep(input, from,            from + halfSize, threshold),
                                    upsweep(input, from + halfSize, end,             threshold))
      Node(left, right)
    } else {
      // return leaf
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    startingAngle: Float, from: Int, lclUntil: Int): Unit = {
    if (from < lclUntil) {
      val maxAngle = max(input(from) / from, startingAngle)
      output(from) = maxAngle
      downsweepSequential(input, output, maxAngle, from + 1, lclUntil)
    }
  }
